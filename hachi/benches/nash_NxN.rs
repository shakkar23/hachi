...
fn bench_nash_60(c: &mut Criterion) {
    let mut rng = rand::rngs::SmallRng::seed_from_u64(42);

    let mut group = c.benchmark_group("nash_equilibrium_60x60");
    group.sample_size(60);           // number of samples for statistics
    group.measurement_time(std::time::Duration::from_secs(30));

    // Warm-up + one representative run
    let payoff = random_payoff_matrix(60, &mut rng);
    ...
    group.bench_function("random_uniform_-1_1", |b| {
        b.iter_batched(
            || random_payoff_matrix(60, &mut rng),
...
criterion_group!(benches, bench_nash_60);
