...
fn test_non_square_tall() {
    ...
    assert!(row[3] < 5e-2, "Dominated row should have near-zero probability, got {}", row[3]);
    // Column play must be ~uniform on the two columns.
    assert!((col[0] - 0.5).abs() < 5e-2);
    assert!((col[1] - 0.5).abs() < 5e-2);
    assert!(value.abs() < 5e-2);
}
...
fn test_non_square_wide() {
    ...
    assert!(col[3] < 5e-2, "Dominated column should have near-zero probability, got {}", col[3]);
    assert!((row[0] - 0.5).abs() < 5e-2);
    assert!((row[1] - 0.5).abs() < 5e-2);
    assert!(value.abs() < 5e-2);
}
